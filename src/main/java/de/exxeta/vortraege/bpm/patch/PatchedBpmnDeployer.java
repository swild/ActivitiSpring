...
public class PatchedBpmnDeployer extends BpmnDeployer {

    private static final Logger LOG = LoggerFactory.getLogger(PatchedBpmnDeployer.class);
    public static final String PATCHED_BPMN_RESOURCE_SUFFIX = "bpmn";
    ...
    public void deploy(DeploymentEntity deployment) {
        ...
        for (String resourceName : resources.keySet()) {
            ...
            if (resourceName.endsWith(PATCHED_BPMN_RESOURCE_SUFFIX)) {
                ResourceEntity resource = resources.get(resourceName);
                byte[] bytes = resource.getBytes();
                ByteArrayInputStream inputStream = new ByteArrayInputStream(bytes);

                BpmnParse bpmnParse = bpmnParser.createParse().sourceInputStream(inputStream).deployment(deployment)
                        .name(resourceName);
                ...
                for (ProcessDefinitionEntity processDefinition : bpmnParse.getProcessDefinitions()) {
                    ...
                    String diagramResourceName = getDiagramResourceForProcess(resourceName, processDefinition.getKey(),
                            resources);
                        ...
                        try {
                            ...
                            if (!ge.isHeadlessInstance()) {
                                byte[] diagramBytes = IoUtil.readInputStream(
                                        ProcessDiagramGenerator.generatePngDiagram(processDefinition), null);
                                diagramResourceName = getProcessImageResourceName(resourceName,
                                        processDefinition.getKey(), "png");
                                createResource(diagramResourceName, diagramBytes, deployment);
                            } else {
                                LOG.warn("Cannot generate process diagram while running in AWT headless-mode");
                            }
                        } catch (Throwable t) { // if anything goes wrong, we
                                                // don't store the image (the
        ...
        for (ProcessDefinitionEntity processDefinition : processDefinitions) {
            if (deployment.isNew()) {
                ...
                ProcessDefinitionEntity persistedProcessDefinition = processDefinitionManager
                        .findProcessDefinitionByDeploymentAndKey(deploymentId, processDefinition.getKey());
    ...
    private void removeObsoleteTimers(ProcessDefinitionEntity processDefinition) {
        List<Job> jobsToDelete = Context.getCommandContext().getJobManager()
    ...
    protected void removeObsoleteMessageEventSubscriptions(ProcessDefinitionEntity processDefinition,
        ...
        if (latestProcessDefinition != null) {
            ...
            List<EventSubscriptionEntity> subscriptionsToDelete = commandContext.getEventSubscriptionManager()
    ...
    protected void addMessageEventSubscriptions(ProcessDefinitionEntity processDefinition) {
        CommandContext commandContext = Context.getCommandContext();
        List<MessageEventDefinition> messageEventDefinitions = (List<MessageEventDefinition>) processDefinition
                .getProperty(BpmnParse.PROPERTYNAME_MESSAGE_EVENT_DEFINITIONS);
            ...
            for (MessageEventDefinition messageEventDefinition : messageEventDefinitions) {
                if (messageEventDefinition.isStartEvent()) {
                    ...
                    List<EventSubscriptionEntity> subscriptionsForSameMessageName = commandContext
                            .getEventSubscriptionManager().findEventSubscriptionByName(MessageEventHandler.TYPE,
                                    messageEventDefinition.getName());
                    // also look for subscriptions created in the session:
                    List<MessageEventSubscriptionEntity> cachedSubscriptions = commandContext.getDbSqlSession()
                    ...
                    subscriptionsForSameMessageName = commandContext.getDbSqlSession().pruneDeletedEntities(
